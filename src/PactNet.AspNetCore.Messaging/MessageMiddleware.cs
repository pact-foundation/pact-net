...
namespace PactNet.AspNetCore.Messaging
{
    ...
    public class MessageMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            try
            {
                (dynamic metadata, dynamic content, JsonSerializerSettings settings) = Scenarios.InvokeScenario(interactionDescription);

                if (metadata != null)
                {
                    AddMetadataToResponse(context, metadata, settings ?? this.options.DefaultJsonSettings);
                }

                context.Response.StatusCode = (int)HttpStatusCode.OK;

                await WriteToResponseAsync(context, content, settings ?? this.options.DefaultJsonSettings);
            }
            catch (InvalidOperationException)
            {
                await WriteErrorInResponseAsync(context,
                                                "The scenario invocation returned a null object. You must setup your messaging scenario so it returns a non-null object.",
