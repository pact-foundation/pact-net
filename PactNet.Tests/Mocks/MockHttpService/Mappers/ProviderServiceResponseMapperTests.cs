...
namespace PactNet.Tests.Mocks.MockHttpService.Mappers
{
    public class ProviderServiceResponseMapperTests
    {
        ...
        public void Convert_WithResponseContentHeaders_CorrectlyMapsHeaders()
        {
            ...
            var mockHttpBodyContentMapper = Substitute.For<IHttpBodyContentMapper>();
            mockHttpBodyContentMapper.Convert(Arg.Any<byte[]>(), Arg.Any<IDictionary<string, object>>()).Returns(new HttpBodyContent(content: Encoding.UTF8.GetBytes(String.Empty), contentType: new MediaTypeHeaderValue("text/plain") { CharSet = "utf-8" }));
        ...
        public void Convert_WithPlainTextContent_CallsConvertOnHttpBodyContentMapperAndCorrectlyMapsBody()
        {
            ...
            var mockHttpBodyContentMapper = Substitute.For<IHttpBodyContentMapper>();
            mockHttpBodyContentMapper.Convert(Arg.Any<byte[]>(), Arg.Any<IDictionary<string, object>>()).Returns(new HttpBodyContent(content: Encoding.UTF8.GetBytes(content), contentType: new MediaTypeHeaderValue("text/plain") { CharSet = "utf-8" }));
            ...
            mockHttpBodyContentMapper.Received(1).Convert(Arg.Is<byte[]>(x => CompareBytes(Encoding.UTF8.GetBytes(content), x)), Arg.Any<Dictionary<string, object>>());
        }
        ...
        public void Convert_WithJsonContent_CallsConvertOnHttpBodyContentMapperAndCorrectlyMapsBody()
        {
            ...
            var mockHttpBodyContentMapper = Substitute.For<IHttpBodyContentMapper>();
            mockHttpBodyContentMapper.Convert(Arg.Any<byte[]>(), Arg.Any<IDictionary<string, object>>()).Returns(new HttpBodyContent(content: Encoding.UTF8.GetBytes(content), contentType: new MediaTypeHeaderValue("application/json") { CharSet = "utf-8" }));
