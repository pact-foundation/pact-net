...
namespace PactNet.Mocks.MockHttpService
{
    public class MockProviderService : IMockProviderService
    {
            ...
            string host = null)
        {
            ...
            BaseUri = new Uri(
                $"{(enableSsl ? "https" : "http")}://{host ?? "localhost"}:{port}");
            _adminHttpClient = adminHttpClientFactory(BaseUri);
        }

        public MockProviderService(int port, bool enableSsl, string consumerName, string providerName, PactConfig config, string host = null)
            : this(
