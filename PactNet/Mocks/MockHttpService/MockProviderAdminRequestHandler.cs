    ...
    internal class MockProviderAdminRequestHandler : IMockProviderAdminRequestHandler
    {
        ...
        public ResponseWrapper Handle(IRequestWrapper request)
        {
            ...
            if (String.IsNullOrEmpty(_mockProviderRepository.TestContext) &&
                ...
                request.Headers.Any(x => x.Key == Constants.AdministrativeRequestTestContextHeaderKey))
            {
                _mockProviderRepository.TestContext = request.Headers.Single(x => x.Key == Constants.AdministrativeRequestTestContextHeaderKey).Value.Single();
            ...
            if (request.Method.Equals("DELETE", StringComparison.OrdinalIgnoreCase) &&
                request.Path == Constants.InteractionsPath)
            {
            ...
            if (request.Method.Equals("POST", StringComparison.OrdinalIgnoreCase) &&
                request.Path == Constants.InteractionsPath)
            {
                return HandlePostInteractionsRequest(request);
            }

            if (request.Method.Equals("GET", StringComparison.OrdinalIgnoreCase) &&
                request.Path == Constants.InteractionsVerificationPath)
            {
                return HandleGetInteractionsVerificationRequest();
            }

            if (request.Method.Equals("POST", StringComparison.OrdinalIgnoreCase) &&
                request.Path == Constants.PactPath)
            {
                return HandlePostPactRequest(request);
            }

            return GenerateResponse(HttpStatusCode.NotFound,
                String.Format("The {0} request for path {1}, does not have a matching mock provider admin action.", request.Method, request.Path));
        }

        private ResponseWrapper HandleDeleteInteractionsRequest()
        {
        ...
        private ResponseWrapper HandlePostInteractionsRequest(IRequestWrapper request)
        {
            var interactionJson = Encoding.UTF8.GetString(request.Body);
        ...
        private ResponseWrapper HandleGetInteractionsVerificationRequest()
        {
        ...
        private ResponseWrapper HandlePostPactRequest(IRequestWrapper request)
        {
            var pactDetailsJson = Encoding.UTF8.GetString(request.Body);
        ...
        private ResponseWrapper GenerateResponse(HttpStatusCode statusCode, string message, string contentType = "text/plain")
        {
            return new ResponseWrapper
            {
                StatusCode = statusCode,
                Headers = new Dictionary<string, string> { { "Content-Type", contentType } },
                Contents = Encoding.UTF8.GetBytes(message)
            };
        }
    }
}